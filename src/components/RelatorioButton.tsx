...
export function RelatorioButton({ transacoes, transacoesFiltradas, mesSelecionado, nomeUsuario }: RelatorioButtonProps) {
    ...
    const calcularStats = () => {
        ...
        const entries = transacoesFiltradas.filter(tx => tx.type === 'entrada').reduce((acc, tx) => acc + Number(tx.amount), 0);
        const exits = transacoesFiltradas.filter(tx => tx.type === 'saida').reduce((acc, tx) => acc + Number(tx.amount), 0);
        return { saldo, entradas: entries, saidas: exits };
    };

    const gerarPDF = async () => {
        alert("ESTE É O CÓDIGO NOVO!");
        const doc = new jsPDF();
